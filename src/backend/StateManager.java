...
public class StateManager {
    ...
    Register getFactorReg(LValue factor) {
        assert valueGetter.getUseValues(localActive.getNowInstrument()).contains(factor);
        // 先找已经有的
                    ...
                    .filter(entry -> {
                            if(isBusy(entry.getKey())){
                                ...
                                return // 如果是结果，则不需要排除
                                        ...
                                        entry.getValue().stream().noneMatch(value -> valueGetter.getUseValues(localActive.getNowInstrument()).contains(value));
                            }
