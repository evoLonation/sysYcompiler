 ...
 * 仅适用于局部变量的声明
 */
public class DeclGenerator extends SequenceGenerator {
    ...
    private void assignment0(Ident ident, IntInitVal initVal, boolean isConst){
        if(initVal == null){
            ...
            if(isConst){
                ...
                symbolTable.newInteger(ident, false);
                addSequence(new Assignment(valueFactory.newVariable(ident), rValue));
            }
        }
    }
    ...
    private void initArray(Ident ident, List<RValue> results) {
        for(int i = 0; i < results.size(); i++){
            ...
            addSequence(new Store(address, results.get(i)));
        }
    }
